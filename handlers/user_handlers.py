...
async def final_cancel_booking(callback: CallbackQuery):
    """
    Финальная отмена билета с новой логикой возврата,
    которая разрешает отмену бесплатных билетов в любое время.
    """
    ...
    # Получаем данные для принятия решения
    payment_id_from_db = order[3]
    amount_str = str(order[5])
    confirmation_text = ""
    was_cancelled = False  # Флаг, чтобы знать, нужно ли обновлять статусы

    # --- НОВАЯ ЛОГИКА ПРОВЕРОК И ВОЗВРАТОВ ---

    # ПРОВЕРКА 1: Это билет, который МОЖНО вернуть в любое время
    # (изначально бесплатный или по промокоду)
    if payment_id_from_db == 'generated_ticket':
        confirmation_text = f"Запись на мероприятие '{event['ShortName']}' отменена!"
        was_cancelled = True

    # ПРОВЕРКА 2: Это билет, который НЕЛЬЗЯ вернуть просто так (платный или подарочный)
    else:
        # Для этих типов билетов мы должны проверить 48-часовое окно
        time_diff = event['datetime_obj'] - datetime.now()
        if time_diff.total_seconds() <= 48 * 3600:
            await callback.answer("упс, до мероприятия осталось менее 48 часов, отмена уже невозможна 🥲", show_alert=True)
            # Возвращаем пользователя к предыдущему экрану, чтобы он не застрял
            await callback.message.edit_text(
                f"твой билетик на мероприятие:\n**{event['ShortName']}**\n\n"
            ...
            return # Прерываем выполнение, так как отмена невозможна

        # Если 48 часов не прошло, продолжаем логику возврата денег/баллов

        # СЦЕНАРИЙ 2А: Отменяется "подарочный" билет
        ...
        # СЦЕНАРИЙ 2Б: Отменяется платный билет
        else:
            try:
    ...
    # Обновляем статусы в наших системах, только если отмена прошла успешно
    if was_cancelled:
